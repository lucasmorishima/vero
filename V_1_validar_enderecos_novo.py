"""
V_1_validar_enderecos_novo.py
-----------------------------
Versão paralelizada de validar_enderecos_novo.py.
Processa todas as linhas com ThreadPoolExecutor(max_workers=10).
...
Saída: arquivos .sql com INSERTs para:
  · V_1_validacao_enderecos
  · V_1_validacao_dados_cadastrais
  · V_1_validacao_status_fatura
"""
...
# Lock for DNE one-time initialization
_dne_load_lock = threading.Lock()

# Thread-safe CEP result cache
_cep_cache: dict = {}
...
def _carregar_dne() -> dict:
    global _dne_index
    # Fast path — already loaded (no lock needed for read after init)
...
def _consultar_cep_cached(cep: str) -> dict:
    """Thread-safe cached wrapper around consultar_cep. Avoids duplicate API calls for the same CEP."""
    with _cep_cache_lock:
        if cep in _cep_cache:
            return _cep_cache[cep]
    # Compute outside the lock — two threads may race for the same CEP on first access,
    # resulting in at most one redundant call per unique CEP, which is acceptable.
...
# ---------------------------------------------------------------------------
# Per-row worker (runs inside ThreadPoolExecutor)
# ---------------------------------------------------------------------------
...
def _processar_linha(args: tuple) -> tuple:
    ...
    Parâmetros
    ----------
    args : (i, row_dict, col_map, total)
        i        – índice original da linha no DataFrame (usado para ordenação)
        row_dict – row.to_dict() com os valores da linha
        col_map  – dict com os nomes das colunas mapeados por papel semântico
        total    – total de linhas (para o log de progresso)

    Retorna
    -------
    (i, linha_val, linha_cad, linha_rel, log_msg)
        linha_val  – dict para validacao_enderecos, ou None
        linha_cad  – dict para validacao_dados_cadastrais, ou None
        linha_rel  – dict para validacao_status_fatura, ou None
        log_msg    – string de progresso a ser impressa em ordem após a conclusão
    """
...
def processar_planilha(arquivo_entrada: str | Path | None = None) -> None:
    ...
    total = len(df)
    print(f"\nProcessando {total} registros | CEP via {fonte_ativa} | workers=20 | brasilapi_sem=1\n")
    ...
    # Submit all rows in parallel; collect results indexed by original row index
    resultados: dict[Any, tuple] = {}
    ...
    # Rebuild output lists in original row order and print logs in order
    ...
    # -----------------------------------------------------------------------
    # Gera arquivos SQL com INSERTs (V_1_ prefix)
    # -----------------------------------------------------------------------
    arq_val_sql = pasta_saida / "V_1_validacao_enderecos.sql"
    _gerar_inserts("validacao_enderecos", linhas_validacao, arq_val_sql)

    arq_cad_sql = pasta_saida / "V_1_validacao_dados_cadastrais.sql"
    _gerar_inserts("validacao_dados_cadastrais", linhas_cadastrais, arq_cad_sql)

    arq_rel_sql = pasta_saida / "V_1_validacao_status_fatura.sql"
