...
_total = sdf.count()               # trigger para materializar o persist
...
sdf_enriched.persist(StorageLevel.MEMORY_AND_DISK)
_n_enriched = sdf_enriched.count()   # trigger para materializar
sdf.unpersist()                        # base sdf não é mais necessário
print(f"sdf_enriched materializado: {_n_enriched} linhas. sdf base liberado.")
...
validacao_dados_cadastrais = sdf_enriched.filter(F.col("_doc_tipo") == "CNPJ").select(
    ...
    F.col("r_cnae_principal_codigo").cast(LongType()).alias("CNAE_Principal_Codigo"),
...
# Libera sdf_enriched — não é mais necessário após os writes
sdf_enriched.unpersist()
